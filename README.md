...
| 4 | Game logic port — module by module in dependency order ([§14](flash-to-html5-conversion-plan.md)) | 🟡 in progress — **Worlds 1 & 2 complete + chained** (displacement0–3, velocity0–3) plus the three walk-across cutscenes. Engine layer, `Avatar`, `Movements` (calibrations v1–v21), `Graph` / `Orb` / `CurveGround` (incl. velocity orbs plotting `vx`), `Spike`, `Switch`, `MovingPlatform`, multi-orb `LevelConfig`, `LevelManager`. Pending: **mixed world (mixed0–3)** + game-ending. |
...
**Latest milestone:** `npm run dev` boots into the main menu; Start plays the intro video (with narration) and runs the full chain — `cutsceneDisplacement → d0–d3 → cutsceneVelocity → v0–v3 → cutsceneMixed`. Completing a level plays the "wisp obtained" cutscene before auto-advancing. Worlds 1 (displacement) and 2 (velocity, incl. the mixed-orb v3) are complete; the menu/options/cutscene/narration framework is in place behind a `SHOW_DEBUG_UI` flag. Only the mixed world (mixed0–3) + ending remain. Collision survived a multi-day calibration journey (see [`docs/calibration.md`](docs/calibration.md) §9 v1–v21). Bundle is ~676 KB (285 KB gzipped); 138/138 unit tests passing. The current handoff is in [`STATUS.md`](STATUS.md).
