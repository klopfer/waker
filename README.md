...
| 4 | Game logic port — module by module in dependency order ([§14](flash-to-html5-conversion-plan.md)) | 🟡 nearly done — **All 11 gameplay levels built + chained** (displacement0–3, velocity0–3, **mixed0–3**) plus the three walk-across cutscenes. Engine layer, `Avatar`, `Movements` (calibrations v1–v21), `Graph` / `Orb` / `CurveGround` (incl. velocity orbs plotting `vx`), `Spike`, `Switch`, `MovingPlatform`, multi-orb `LevelConfig`, `LevelManager`. Pending: game-ending video + playtest calibration of v2/v3 and mixed0–3. |
| 5 | UI port — DOM overlay (menu, options, instructions, credits, HUD) | 🟡 in progress — main menu, instructions/credits screens, options (difficulty + Music/SFX toggles), intro video, cutscene narration + "wisp obtained" win cutscene, boot flow all done. Pending: in-game pause menu, studio splash logos. |
| 6 | Testing + polish (cross-browser, mobile, perf) | ⏳ |
| 7 | Release prep | ⏳ |

**Latest milestone:** all three worlds + cutscenes are built and chained — `cutsceneDisplacement → d0–d3 → cutsceneVelocity → v0–v3 → cutsceneMixed → mixed0–mixed3`. `npm run dev` boots into the main menu; Start plays the intro video (with narration) and runs the full chain end-to-end. Completing a level plays the "wisp obtained" cutscene before auto-advancing. Menu/options/cutscene/narration framework is in place behind a `SHOW_DEBUG_UI` flag. mixed3 is the terminal level (game-ending video pending). Collision survived a multi-day calibration journey (see [`docs/calibration.md`](docs/calibration.md) §9 v1–v21). Bundle is ~676 KB (285 KB gzipped); 138/138 unit tests passing. The current handoff is in [`STATUS.md`](STATUS.md).
